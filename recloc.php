<?php
 require_once "config.php";   
     if(!isset($_SESSION)){ session_start();}
      $ctr=0;
      //SELECT COUNT(`regid`), `location` FROM `tbl_contracing` GROUP BY `location`
      //SELECT * FROM `tbl_contracing` where `location`='2'
      //SELECT COUNT(`regid`), `location` FROM `tbl_contracing` where hdate='2020-09-07' GROUP BY `location`

      date_default_timezone_set('Asia/Manila');
      $today = strtotime(date("y-m-d h:i:sa"));
      $hdate= date("Y-m-d", $today);

      $query="SELECT COUNT(regid) as regid, location FROM tbl_contracing where hdate='$hdate' GROUP BY location";
      $dtfrm='$hdate';

      if($_SERVER["REQUEST_METHOD"]=="POST"){ 
        if (isset($_POST['filter'])) {
          
              $query="SELECT COUNT(regid) as regid, location FROM tbl_contracing where hdate='$_POST[dtfrm]' GROUP BY location";
              $dtfrm=$_POST['dtfrm'];
...
<body>
     <h1 style="text-align:center;"> Most visited locations </h1>
  ...
  <div class="btn-group pull-left" style="padding-top:10px; padding-left: 20px;" >
    <form class="form-inline" method="POST">
      ...
      Date: <input type="date" name="dtfrm" id="dtfrm" value="<?php echo $dtfrm; ?>" max="<?php echo $hdate; ?>">
  ...
  <div class="fresh-table full-screen-table">
    ...
    <table id="fresh-table" class="table">
      <thead>
        ...
        <th data-field="vitcount" data-sortable="true">Visitors count</th>
